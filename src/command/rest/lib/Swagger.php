    ...
    protected function getTpl()
    {
        ...
        foreach ($arr as $k => $v) {
            if ($this->input->getOption($v) or $this->input->hasParameterOption('-'.$k)) {
                $$v = file_get_contents(root_path(). DIRECTORY_SEPARATOR . 'vendor'.DIRECTORY_SEPARATOR.'orchief'.DIRECTORY_SEPARATOR.'utility'.DIRECTORY_SEPARATOR.'src' .DIRECTORY_SEPARATOR .  'command'. DIRECTORY_SEPARATOR .'tpl/swagger_'.$v.'.tpl');
            }
            ...
            foreach ($arr as $kk => $vv) {
                if ($str && strpos($str, $kk) !== false) {
                    $$vv = file_get_contents(root_path(). DIRECTORY_SEPARATOR . 'vendor'.DIRECTORY_SEPARATOR.'orchief'.DIRECTORY_SEPARATOR.'utility'.DIRECTORY_SEPARATOR.'src' .DIRECTORY_SEPARATOR .  'command'. DIRECTORY_SEPARATOR .'tpl/swagger_'.$vv.'.tpl');
                }
            }
        }
